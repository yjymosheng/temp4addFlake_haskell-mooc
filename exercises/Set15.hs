...
--
-- 练习1：使用 Applicative 操作（即 liftA2 和 pure）对两个 Maybe Int
-- 值求和。不要使用模式匹配。
--
-- Examples:
...
-- of the form "x is [not] y". Use Applicative operations like liftA2!
-- The order of the results doesn't matter.
--
-- 练习2：给定两个单词列表 xs 和 ys，生成所有形如 "x is [not] y" 的语句。
-- 使用 Applicative 操作如 liftA2！结果的顺序不重要。
--
-- Examples:
...
statements xs = liftA2 (++) (liftA2 (++) xs [" is ", " is not "])
...
--
-- 练习3：一个带错误处理的简单计算器。给定一个操作（negate 或 double）
-- 和一个数字，均为字符串，计算结果。对于未知操作或无效数字返回 Nothing。
-- 使用 Applicative 操作，不要使用模式匹配。提示：记住函数 readMaybe
--
-- Examples:
...
calculator k v  = lookup k [("negate", negate), ("double", (* 2))] <*>  readMaybe v
...
--
-- 练习4：安全除法。实现函数 validateDiv，将两个整数相除，但如果除数为零
-- 则返回错误（"Division by zero!"）。
-- 注意！Validation 的构造器没有被导出，所以你不能对 Validation 进行
-- 模式匹配，你必须使用 Applicative 方法以及 invalid 和 check 函数。
--
-- Examples:
...
validateDiv a b  = div a <$> check (b /= 0) "Division by zero!" b
...
-- * Street length should be at most 20 characters (if not, error "Invalid street name")
-- * Street number should only contain digits (if not, error "Invalid street number")
-- * Postcode should be exactly five digits long (if not, error "Invalid postcode")
--
-- 练习5：验证街道地址。街道地址由街道名称、街道号码和邮政编码组成。
-- 实现函数 validateAddress，如果输入有效则构造一个 Address 值：
-- * 街道名称长度最多为 20 个字符（否则，错误 "Invalid street name"）
-- * 街道号码应只包含数字（否则，错误 "Invalid street number"）
-- * 邮政编码应恰好为五位数字（否则，错误 "Invalid postcode"）
--
-- Examples:
...
validateAddress streetName streetNumber postCode =
  Address <$> checkName <*> checkNum <*> checkCode
  where
    checkName = check (length streetName <= 20) "Invalid street name" streetName
    checkNum  = check (all isDigit streetNumber) "Invalid street number" streetNumber
    checkCode = check (length postCode == 5 && all isDigit postCode) "Invalid postcode" postCode
...
--
-- 练习6：给定两个人的姓名、年龄和就业状态，包装在 Applicative 中，返回一个
-- 包含两个 Person 值的列表，也包装在 Applicative 中。
--
-- Examples:
...
twoPersons :: Applicative f =>
  ...
  -> f [Person]
twoPersons name1 age1 employed1 name2 age2 employed2 = liftA2 (\a b -> [a ,b ]) (Person <$> name1 <*> age1 <*> employed1) (Person <$> name2 <*> age2 <*> employed2)
...
-- (0x3a) or octal (0o14).
--
-- 练习7：验证一个字符串是 Bool 还是 Int。函数的返回类型使用 Either Bool Int
-- 来表示两种情况。使用 <|> 组合两个验证器，如果值既不是 Int 也不是 Bool，
-- 则产生两个错误。提示：记住 readMaybe。
-- 附：测试不会测试 Int 字面量的特殊情况，如十六进制（0x3a）或八进制（0o14）。
--
-- Examples:
...
boolOrInt s = boolValidator <|> intValidator
  where
    boolValidator = maybe (invalid "Not a Bool") (pure . Left)  (readMaybe s :: Maybe Bool)
    intValidator  = maybe (invalid "Not an Int")  (pure . Right) (readMaybe s :: Maybe Int)
...
--
-- 练习8：改进的电话号码验证。实现函数 normalizePhone，给定一个字符串：
-- * 移除字符串中的所有空格
-- * 检查剩余字符最多为 10 个
-- * 检查所有剩余字符是否为数字，并为每个无效字符记录一个错误
-- * 如果没有错误，返回去除空格后的字符串
--
-- Examples:
...
normalizePhone s = liftA2 (\_ x -> x ) (checkLen s) (checkDigit (remove s) )
  where
    remove [] = []
    remove (x:xs) = if x == ' ' then remove xs else x: remove xs
    checkLen s = check ( length ( remove s )  <= 10   )  "Too long" ( remove s )
    checkDigit []  =  pure []
    checkDigit (x : xs )= liftA2 (:) (check (isDigit x ) ("Invalid character: " ++ [x]  ) x) (checkDigit xs )
...
--   "Invalid expression: 1 + 2 +" / "Invalid expression: 1 -"
...
--
-- 练习9：解析表达式。Expression 类型描述一种具有运算符（+ 或 -）和两个参数
-- 的算术表达式，参数可以是数字或单字母变量。运算符和参数之间总是用空格分隔。
-- 以下是此类表达式的一些示例：1 + 2, y + 7, z - w。
-- 实现函数 parseExpression，使用 Validation applicative 将 "y + 7" 这样的字符串
-- 转换为 Plus (Variable 'y') (Number 7) 这样的 Expression 值。
-- 解析器应产生以下错误：
-- * 对于非 + 或 - 的运算符："Unknown operator: %"
-- * 对于非单字母的变量："Invalid variable: xy"
-- * 对于非数字的参数："Invalid number: 1x"
-- * 对于不包含三个单词的表达式："Invalid expression: 1 + 2 +" / "Invalid expression: 1 -"
-- 提示：函数 `words` 和 `isAlpha`。
-- 提示：如果你在错误顺序上有问题，请记住 Validation 从左到右收集错误！
--
-- Examples:
...
parseExpression s = case words s of
  [a, op, b] -> opfn op <*> f a <*> f b
  _ -> invalid ("Invalid expression: " ++ s)
  where
    opfn op = check (op `elem` ["+", "-"])
                     ("Unknown operator: " ++ op)
                     (if op == "+" then Plus else Minus )

    f w = numV <|> varV
      where
        numV = maybe (invalid ("Invalid number: " ++ w))
                     (pure . Number)
                     (readMaybe w :: Maybe Int)
        varV = case w of
...
--
-- 练习10：Priced T 类型跟踪一个类型为 T 的值和一个价格（由 Int 表示）。
-- 实现 Priced 的 Functor 和 Applicative 实例。它们应该这样工作：
...
--
-- 练习11：本练习和下一个练习将使用 Applicative 类型类的副本 MyApplicative。
-- MyApplicative 缺少 Applicative 所需的 Functor 约束，以及 <*> 类型类方法。
-- 你需要自己实现它们。
-- 首先你将使用 liftA2 重新实现 <*>。具体来说，实现 <#> 操作符，其功能类似 <*>，
-- 使用 myPure 和 myLiftA2。
-- 只要类型正确，你的实现基本上就保证是正确的。
--
-- Examples:
...
--
-- 练习12：使用 liftA2 和 pure 重新实现 fmap。具体来说，使用 MyApplicative
-- 类型类中的 myPure 和 myLiftA2 方法实现下面的函数 myFmap。
-- 只要类型正确，你的实现基本上就保证是正确的。不过，这次有几种不同的可能实现！
--
-- Examples:
...
--
-- 练习13：给定一个返回 Alternative 值的函数和一个列表，对该列表中的所有元素
-- 尝试该函数，并产生所有成功的结果。
-- 提示：traverse 不会帮助你，因为它只有在所有调用都成功时才成功。
-- 你需要使用 <|>。
--
-- Examples:
-- The Maybe Applicative returns the first success（Maybe Applicative 返回第一个成功的结果）:
--   tryAll (\x -> if x>0 then pure x else empty) [0,3,2] :: Maybe Int
--     ==> Just 3
--   tryAll (\x -> if x>0 then pure x else empty) [0,-1,0] :: Maybe Int
--     ==> Nothing
-- The list Applicative returns all successes（列表 Applicative 返回所有成功的结果）:
--   tryAll (\x -> if x>0 then pure x else empty) [0,3,2] :: [Int]
--     ==> [3,2]
-- The Validation Applicative returns the first success or all errors（Validation Applicative 返回第一个成功结果或所有错误）:
...
-- both Functors.
--
-- 练习14：这里有一个类型 `Both`，它表示函子的组合。以下是一些示例值和类型：
--   Both (Just [True])                    :: Both Maybe [] Bool
--   Both [Just True, Nothing, Just False] :: Both [] Maybe Bool
--   Both [[True,False],[]]                :: Both [] [] Bool
--   Both (Ok (Just "value"))       :: Both Validation Maybe String
--   Both (Just (Errors ["wrong"])) :: Both Maybe Validation a
-- 为 Both f g 实现 Functor 实例，假设 f 和 g 都是 Functor。
--
-- Examples:
...
instance (Functor f, Functor g) => Functor (Both f g) where
  fmap x (Both a ) = Both (fmap (fmap x )  a )
...
-- right.
--
-- 练习15：两个 Applicative 的组合也是一个 Applicative！实现 Applicative
-- (Both f g) 实例（假设 f 和 g 已经是 Applicative）。同样，只有一种实现方式
-- 能让类型正确。
--
-- Examples:
