...
import Mooc.Todo
import Data.Char (toLower, GeneralCategory (Space))
...
instance Price Milk where
  price  (Milk a) = fromIntegral $  15 * a
...
instance Eq RationalNumber where
  (RationalNumber a b ) == (RationalNumber x y ) =a * y == b * x
...
simplify (RationalNumber a b )  =  RationalNumber  (a `div` x ) (b `div` x )
  where
    x  =  gcd a b 
...
instance Num RationalNumber where
  (RationalNumber a b ) + (RationalNumber x y) =  simplify $ RationalNumber (a*y + x *b ) (b * y )
  (RationalNumber a b ) * (RationalNumber x y) = simplify  $ RationalNumber (a* x) (b * y)
  abs (RationalNumber x y) = simplify $ RationalNumber (abs x)   y 
  signum (RationalNumber a b) =  RationalNumber (signum a ) (signum b)
  fromInteger x = RationalNumber x  1 
  negate (RationalNumber x y) = simplify $ RationalNumber (-x ) y 
